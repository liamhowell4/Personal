 ...
 *  @author Kevin Wayne
 *  @source https://introcs.cs.princeton.edu/
 */
public final class StdAudio {
    ...
    private static void init() {
        try {
            ...
            // this choice is arbitrary it gets divided because we can't
            // expect the buffered data to line up exactly with when
    ...
    private static AudioInputStream getAudioInputStreamFromFile(String filename) {
        ...
        try {
            ...
            throw new IllegalArgumentException("file of unsupported audio format: '" + filename
                    + "'", e);
        }
    }
    ...
    public static double[] read(String filename) {
        ...
        if (audioFormat.getSampleRate() != SAMPLE_RATE) {
            throw new IllegalArgumentException("StdAudio.read() currently supports only a "
                    + "sample rate of " + SAMPLE_RATE + " Hz\n"
        ...
        if (audioFormat.isBigEndian()) {
            throw new IllegalArgumentException("StdAudio.read() currently supports only "
                    + "audio stored using little endian\n"
                    + "audio format: " + audioFormat);
        }
        ...
        try {
            ...
            if (bytesToRead != bytesRead) {
                throw new IllegalStateException("read only " + bytesRead + " of "
                        + bytesToRead + " bytes");
            }
        ...
        if (audioFormat.getChannels() == MONO) {
            ...
            for (int i = 0; i < n / 2; i++) {
                // little endian, mono
                data[i] = ((short) (((bytes[2 * i + 1] & 0xFF) << 8)
            ...
            for (int i = 0; i < n / 4; i++) {
                double left  = ((short) (((bytes[4 * i + 1] & 0xFF) << 8)
                        | (bytes[4 * i + 0] & 0xFF))) / ((double) MAX_16_BIT);
                double right = ((short) (((bytes[4 * i + 3] & 0xFF) << 8)
                        | (bytes[4 * i + 2] & 0xFF))) / ((double) MAX_16_BIT);
                data[i] = (left + right) / 2.0;
            }
            return data;
        } else {
            throw new IllegalStateException("audio format is neither mono or stereo");
        } // handle big endian (or other formats)
    }
    ...
    private static void stream(AudioInputStream ais) {
        ...
        int bufferSize1 = 4096; // 4K buffer
        ...
        try {
            ...
            line1.start();
            byte[] samples = new byte[bufferSize1];
            int count = 0;
            while ((count = ais.read(samples, 0, bufferSize1)) != -1) {
                line1.write(samples, 0, count);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } catch (LineUnavailableException e) {
